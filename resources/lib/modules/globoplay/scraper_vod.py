...
def get_thumb_offer(id, page=1, per_page=PAGE_SIZE):
    ...
    for item in filter(__filter_plus, resources):
        yield {
                ...
                'year': (item.get('title', {}) or {}).get('releaseYear', ''),
                'country': item.get('countries', []) or [],
                'genre': item.get('genresNames', []) or [],
                'cast': item.get('castNames', []) or [],
                'director': item.get('directorsNames', []) or [],
                'writer': item.get('screenwritersNames', []) or [],
                'credits': item.get('artDirectorsNames', []) or [],
                ...
                'art': {
                    ...
                    'fanart': (item.get('title', {}) or {}).get('cover', {}).get('web', GLOBO_FANART),
...
def get_poster_offer(id, page=1, per_page=PAGE_SIZE):
    ...
    for resource in items.get('resources', []) or []:
        ...
        yield {
            ...
            'country': resource.get('countries', []) or [],
            'genre': resource.get('genresNames', []) or [],
            'cast': resource.get('castNames', []) or [],
            'director': resource.get('directorsNames', []) or [],
            'writer': resource.get('screenwritersNames', []) or [],
            'credits': resource.get('artDirectorsNames', []) or [],
...
def get_excerpts_by_date(id, gte, lte):
    ...
    for resource in resources:
        ...
        yield {
            ...
            'credits': title.get('artDirectorsNames', []) or [],
            'mpaa': title.get('contentRating'),
            'studio': (title.get('channel', {}) or {}).get('name'),
            'aired': (video.get('exhibitedAt', '') or '').split('T')[0],
            'art': {
                'thumb': video.get('thumb'),
                'fanart': (title.get('cover', {}) or {}).get('landscape', GLOBO_FANART),
